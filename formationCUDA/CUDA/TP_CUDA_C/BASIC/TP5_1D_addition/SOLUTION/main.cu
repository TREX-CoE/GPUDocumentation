...
__global__ void myKernel(int *d_a, int *d_b, int *d_c, int size){
...
int main(int argc, char **argv){
  ...
  nbBlocks.x = (size + threadsPerBlock.x -1) / threadsPerBlock.x;

  myKernel<<< nbBlocks, threadsPerBlock >>>(d_a, d_b, d_c, size);
