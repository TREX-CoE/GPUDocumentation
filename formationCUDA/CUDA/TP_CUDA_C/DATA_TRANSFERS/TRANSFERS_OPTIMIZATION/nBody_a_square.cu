...
__global__ void update_velocity_gpu(int size, float *x, float *y, float *z, float * vx, float *vy, float *vz, float *mass,float softeningSquared, float G, float timestep, float damping)
{
	...
	if( index < size){
		...
		vy[index] = vy[index] + tmpax * timestep * damping;
		vz[index] = vz[index] + tmpax * timestep * damping;
...
__global__ void update_position_gpu(int size, float* x, float *y, float *z, float *vx, float *vy, float *vz, float timestep)
{
...
void alloc_device_data()
{
	...
	cudaMalloc((void**)&d_array_z, size * sizeof(float)); 
...
int main(int argc, char* a[])
{
	...
	cudaMemcpy(d_array_vx, array_vx, size * sizeof(float), cudaMemcpyHostToDevice);
	cudaMemcpy(d_array_vx, array_vy, size * sizeof(float), cudaMemcpyHostToDevice);
	cudaMemcpy(d_array_vx, array_vz, size * sizeof(float), cudaMemcpyHostToDevice);
	cudaMemcpy(d_array_vx, array_mass, size * sizeof(float), cudaMemcpyHostToDevice);


	/* Data transfert */
	cudaMemcpy(d_array_x, array_x, size * sizeof(float), cudaMemcpyHostToDevice);
	cudaMemcpy(d_array_y, array_y, size * sizeof(float), cudaMemcpyHostToDevice);
	cudaMemcpy(d_array_z, array_z, size * sizeof(float), cudaMemcpyHostToDevice);
	...
	for (i=0; i<nb_steps; i++)
	{
		...
		update_velocity_gpu<<<gridsize, blocksize >>>(size, d_array_x, d_array_y, d_array_z, d_array_vx, d_array_vy, d_array_vz, d_array_mass, softeningSquared,G, timestep, damping);
		update_position_gpu<<<gridsize, blocksize >>>(size, d_array_x, d_array_y, d_array_z, d_array_vx, d_array_vy, d_array_vz, timestep);
		//update_position();
	}
	double t1 = wallclock();
	/* Data transfert */
	cudaMemcpy(array_x, d_array_x, size * sizeof(float), cudaMemcpyDeviceToHost);
	cudaMemcpy(array_y, d_array_y, size * sizeof(float), cudaMemcpyDeviceToHost);
	cudaMemcpy(array_z, d_array_z, size * sizeof(float), cudaMemcpyDeviceToHost);

	cudaMemcpy(array_vx, d_array_vx, size * sizeof(float), cudaMemcpyDeviceToHost);
	cudaMemcpy(array_vy, d_array_vy, size * sizeof(float), cudaMemcpyDeviceToHost);
	cudaMemcpy(array_vz, d_array_vz, size * sizeof(float), cudaMemcpyDeviceToHost);
	cudaMemcpy(array_mass, d_array_mass, size * sizeof(float), cudaMemcpyDeviceToHost);
