...
__global__ void  transpose_block(int size_x, int size_y, int *d_in, int *d_out){


  int idx = threadIdx.x + blockDim.x * blockIdx.x;
  int idy = threadIdx.y + blockDim.y * blockIdx.y;
  int index = idx + size_x * idy;
  int index_T = idy + size_y * idx;
  //to complete: d_out = d_in
  if(idx < size_x && idy < size_y)
	  d_out[index_T] = d_in[index];
...
int main(int argc, char **argv){
  ...
  transpose_block<<<gridsize, blocksize>>>(size_x, size_y, d_in, d_out);
  ...
  for(int j=0; j<size_y; j++){
    for(int i=0; i<size_x; i++){
      int idiv32 = i / blocksize.x ;
      int imod32 = i % blocksize.x ;
      int jdiv32 = j / blocksize.y ;
      int jmod32 = j % blocksize.y ;
      int idx_in =  imod32 + jdiv32 * blocksize.y ;
      int idy_in =  jmod32 + idiv32 * blocksize.x ;
      index_in = idx_in + idy_in *size_y; 

      if(h_out[i+j*size_x] != h_in[index_in] ){
	success = 0;
